...
(function(window, document, $, cmb, undefined){
	...
	cmb._handleMedia = function( formfield, isList ) {
		...
		// TODO: Waiting for a fix, https://github.com/CMB2/CMB2/issues/983
		...
		handlers.selectFile = function() {
			...
			// TODO: Waiting update, https://github.com/CMB2/CMB2/issues/985
	...
	// TODO: Waiting for a fix, https://github.com/CMB2/CMB2/issues/989
	...
	cmb.updateNameAttr = function () {
		...
		if ( typeof name !== 'undefined' ) {
			...
			var $newName = cmb.replaceLast( name, '[' + prevNum + ']', '[' + newNum + ']' );
		...
		$goto.find( cmb.repeatEls ).each( function( index ) {
			...
			if ( $element.hasClass('cmb2-media-status') ) {
				...
				inputVals[ index ].$.find( 'input' ).each(function() {
					...
					name = cmb.replaceLast( name, '['+toRowId+']', '['+fromRowId+']' );
					$( this ).attr( 'name', name );
				});
				$element.find('input').each(function() {
					var name = $( this ).attr('name');
					name = cmb.replaceLast( name, '['+fromRowId+']', '['+toRowId+']');
					$( this ).attr('name', name);
				});
